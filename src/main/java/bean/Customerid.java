		...
		try {
			...
			ResultSet rs = ps.executeQuery();
				     while(rs.next())
		            {
		                ...
		                out.println("<header><h3>Customer List</h3> </header>"); 
		                out.println("<section>");  
		                out.println("<nav>");  
		                out.println("<ul>");  
		                out.println("<li>");  
		                ...
		                out.println("</nav>");  
		                out.println("<article>");  
		                out.println("  <h1 style=\"color:white; font-size: 20px;\"> Customer Records</h1>");  
		                out.println("<div class=\"form-popup\" id=\"myForm\" style=\"height:250px;width:1180px;overflow:scroll;overflow-y:scroll;overflow-x:hidden;\">");  
			             ...
			             out.println("<tr> <td style=\"font-weight: bold;\">QUANTITY</td> <td>"  + rs.getString("Quantity")   +  "</td></tr>");
			             ...
			             out.println("</table>");
			             out.println("<tr>"  + "<a style=\"color:black ;\" href='Aduseredit?id=" +  rs.getString("id")  + "'><button style=\"margin: 5px; color:Black; background-color:#03c03c   ; width:100px; height:30px; border-color:#fff;border-radius:20px;\" >Edit</button>  </a>"+ "<a  style=\"color:black;\"  href='Customerdelete?id=" +  rs.getString("id")  + "'><button onclick=\"return confirm('Are you sure you want to reject this request?');\"style=\"margin: 5px; color:Black; background-color:#ff0800 ; width:100px;height:30px; border-color:#fff;border-radius:20px;\">Delete</button>  </a>" + "</tr>");
			             out.println("</div>");  
		                 out.println(" </article>");  
		                 out.println(" <script>\r\n"
		                 		+ "                    function openForm() \r\n"
